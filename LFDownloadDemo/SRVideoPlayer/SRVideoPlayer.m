            ...
            case AVPlayerStatusReadyToPlay:
            {
                ...
                _playbackTimeObserver = [_player addPeriodicTimeObserverForInterval:CMTimeMake(1, 1) queue:NULL usingBlock:^(CMTime time) {
                    ...
                    if (loadedRanges.count > 0)
                    {
                        ...
                        strongSelf.videoDuration = CMTimeGetSeconds(range.start) + CMTimeGetSeconds(range.duration);
                        strongSelf.bottomBar.totalTimeLabel.text = [strongSelf formatTimeWith:(long)ceil(strongSelf->_videoDuration)];
                        strongSelf.bottomBar.playingProgressSlider.minimumValue = 0.0;
                        strongSelf.bottomBar.playingProgressSlider.maximumValue = strongSelf.videoDuration;
                        // 当前播放总时间
                        NSLog(@"duration:%g", strongSelf.videoDuration);
                    }
