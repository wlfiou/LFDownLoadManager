...
+(NSString *)downloadPathDirectory{
    return [[self CachesDirectory] stringByAppendingPathComponent:@"LFDownload"];
}
//resumeData文件目录 (断点下载时每次存储状态数据包)
+(NSString *)downloadResumeDataDirectory{
    return [[self CachesDirectory] stringByAppendingPathComponent:@"LFResumeDataDownload"];
}
//验证手机号码
